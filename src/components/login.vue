<template>
	<section class="login_box" :style="{height:logHeight}" >
		...
		<form class="login_form">
			...
			<input v-model="name" :style="emialObj" type="text" class="login_input" @blur="backBlur" @focus="backFocus"/>
			<label class="login_label">Password:</label>
			<input v-model="pass" :style="passObj" type="text" class="login_input" @blur="passBlur" @focus="passFocus"/>
			<input type="button" class="login_btn" value="登录" @click="login" />
			<div class="login_reg clearfix">
				<span class="fl">忘记密码？</span>
				<span class="fr" @click="goReg">去注册</span>
			</div>
		</form>
	</section>
</template>
		...
		methods:{
			...
			login(){
				...
				$.ajax({
					type:"post",
					url:"/api/login",
					data:{name:self.name,pass:self.pass},
					success:function(data){
						if(data.code==200){
							plus.nativeUI.toast('登录成功');
							self.$store.state.loginStatus = true;
							setCookie('user',{name:self.name,pass:self.pass});
							goTo({path:self.nextPath},self);
						}else{
...
<style>
	...
	.login_form{margin-top: 80px;padding: 0 15px; box-sizing: border-box;}
